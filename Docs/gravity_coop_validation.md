...
## 상자 집기/운반 안정화 (2026-09-19)

- 원인: 운반 sweep 또는 목표 위치 overlap에 장애물이 하나라도 있으면 즉시 Drop을 호출했다. 운반 방향은 시선이 아니라 이동 방향(Facing)이어서 옆걸음·후진에도 상자가 크게 돌아갔다. 집기는 중심 거리 기준이며 레버가 상자보다 무조건 우선했다.
- 변경: 충돌 경로의 안전한 지점까지만 상자를 옮기고 잡기는 유지한다. 장애물이 없어지면 다시 따라온다. 막힌 상자를 두고 멀리 이동해 플레이어-상자 중심 거리가 interactionDistance + 1(기본 3.2)을 넘을 때는 놓는다. 명시적 E, 사망·리셋·구간 완료의 놓기 처리는 유지했다.
- 실제 마우스 입력은 서버에서 역할·유한값을 검증하는 LookRpc로 좌우 시선을 전달한다. 운반 위치는 해당 시선의 중력 평면 방향을 사용하며, 상하 보기로 상자를 들어올리는 기능은 추가하지 않았다. 기존 자동 풀이 입력은 LookRpc를 보내지 않을 때 기존 Facing 기준으로 동작한다.
- 집기 거리와 대상 우선순위는 Collider.ClosestPoint로 계산한 표면 거리로 판단한다. 레버와 상자 중 도달 가능한 가장 가까운 대상을 선택하며 벽 가림 검사는 유지한다.
- `Logs/gravity-carry-build.log`: Unity 6000.3.10f1 개발 빌드 성공. 실행 파일 갱신. `Logs/gravity-carry-host.log`: HOST PASS 1081 checks, 8개 퍼즐 연속 완료. `Logs/gravity-carry-client.log`: CLIENT PASS.
- 추가 검증: 표면 거리 집기, 실제 클라이언트 시선 RPC를 통한 후진/옆걸음 운반, E 놓기, 벽 충돌 시 잡기 유지/벽 통과 방지, 장애물 제거 후 운반 재개.
- 카메라 자동 검사를 네트워크 순간이동과 LateUpdate 사이에 실행하면 거짓 실패가 발생해, 카메라 갱신 이후 검사하도록 보정했다. 실제 마우스 조작감 및 인터넷 Relay 지연은 로컬 두 프로세스 테스트의 검증 범위 밖이다.

## 오프라인 1인 좌우 분할 테스트 (2026-09-19)

- 메인의 혼자 테스트 버튼은 소켓을 열지 않는 OfflineTransport로 동일한 Netcode 권한 시뮬레이션을 단일 프로세스에서 실행한다. 로그인, Relay, 방 생성, 두 번째 클라이언트 없이 시작한다. 중력 게임은 멀티 버튼을 선택할 때만 온라인 서비스를 초기화한다.
- 왼쪽 절반은 중력 조작자 전체 시점, 오른쪽 절반은 내부 플레이어 1인칭이다. 화살표와 WASD/마우스 입력을 같은 인스턴스에서 처리한다. E/Space/R도 기존 규칙을 따른다. 화면별 렌더 콜백으로 왼쪽에는 캐릭터를 표시하고 오른쪽에서는 숨기며, 표식도 각각의 카메라를 향한다. 카메라 aspect는 각 viewport의 실제 픽셀 비율을 사용한다.
- Esc 설정의 테스트 종료 · 모드 선택으로 나가면 씬을 초기화해 추가 카메라와 오프라인 전송을 정리하고 멀티 버튼을 복원한다. 모드 변경 시 진행 상태는 초기화된다. 멀티 역할 권한은 그대로 유지한다.
- `Logs/gravity-solo-regression.log`: 1개 프로세스에서 HOST PASS 1091 checks, 8개 퍼즐 완료. 오프라인 전송, 단일 참가자, 좌우 카메라, 설정 진입/복귀와 운반 회귀 검사 포함.
- `Logs/gravity-solo-multi-host.log`: HOST PASS 1088 checks, 기존 2인 로컬 멀티 8개 퍼즐 완료. `Logs/gravity-solo-multi-client.log`: CLIENT PASS.
- `Logs/gravity-solo-visual.log`: 실제 버튼으로 혼자 테스트 진입·종료 2회, 온라인 서비스 미초기화, 오디오 리스너 1개, 카메라 정리, 멀티 전송 복원 검사. 숨겨진 창의 백버퍼가 검게 캡처되어 UI/카메라를 RenderTexture에 직접 그려 확인한다. `Logs/gravity-solo-menu.png`, `gravity-solo-split.png`, `gravity-solo-settings.png`가 검토용 이미지다.
- `Logs/gravity-solo-build.log`: Unity 6000.3.10f1 개발 빌드. 실행 파일은 `Builds/GravityPrototype/GravityPrototype.exe`로 갱신한다. 개발 자동 검사는 `-gravitySmoke -gravitySolo`, UI 전환 검사는 `-gravitySoloModeTest`로 실행할 수 있다.
- 자동 입력은 기존 RPC 경로를 이용한다. 실제 키보드/마우스 체감과 인터넷 Relay 왕복은 이번 자동 검사의 범위 밖이다.

## 새 기믹 스테이지 9~12 (2026-09-19)

- 기존 8개 뒤에 앞뒤 인계, 회전 통로, 상자 고정, 종합 배송팀을 추가했다. 9번의 A 바닥 창구와 B 천장 창구는 모두 사용 가능하다. 10번은 실제 충돌체 두 개로 구성한 ㄱ자 통로 2개를 90도 전환한다. 11번은 고정한 P를 유지한 채 Q를 배송한다. 12번은 P 고정 → 인계문 개방 → 뒤쪽 운반 → 회전 배송으로 이어진다.
- 새 장치는 기존 서버 권한 E 상호작용의 거리·가림·가장 가까운 대상 선택을 사용한다. 고정된 상자는 집기에서 제외한다. 회전은 내부 점유와 도착 벽의 물체 겹침을 검사한다. 고정은 놓여 있고 완전히 소켓 안에 들어온 느린 상자만 허용한다. 회전 방향, 거절 상태, 잠긴 상자, 중간 문, 체크포인트를 NetworkVariable로 동기화한다.
- 12번에서 P 잠금 후 R/실패 재시도는 P 잠금만 복원하고 나머지 상자·통로·사람·중력은 초기 상태로 돌린다. Shift+R은 체크포인트까지 초기화한다. 열린 인계문에 사람이 있으면 압력판이 풀려도 닫히지 않는다.
- 기존 작업 중이던 1인 오프라인 모드, 1인칭 시점, 시선 기준 운반 변경을 보존했다. 관련 파일의 작업 전 사본은 무시된 `Logs/expansion-baseline`에 보관했다.
- `Logs/gravity-expansion-build.log`: 최종 Unity 6000.3.10f1 Windows 개발 빌드 성공, 종료 코드 0. 12개 방, 2078개 컴포넌트의 누락 스크립트/참조 없음. 회전 통로와 고정 장치의 필수 참조 검사도 추가했다.
- `Logs/gravity-twelve-host.log`: HOST PASS 2215 checks. 기존 1~8번과 새 9~12번 연속 완료. 9번은 초기 상태에서 A/B 두 경로를 각각 클리어했다. 새 방의 참고 풀이는 위치 강제 이동 없이 클라이언트 이동·시선·E RPC와 호스트 중력 전환으로 실행했다.
- 별도 설정을 사용하는 안전 검사는 상자/사람이 들어 있는 회전 통로, 도착 벽의 겹침, 빈 소켓, 운반 중인 상자, 움직이는 상자의 고정 거절, 잠긴 상자 집기 방지, 고정 유지/해제, 중간 체크포인트 복원과 완전 초기화를 포함한다.
- `Logs/gravity-twelve-client.log`: CLIENT PASS. 회전 방향, 고정 상태, 체크포인트 수신과 마지막 출구·완료 상태 동기화 확인.
- `Logs/gravity-expansion-solo.log`: 오프라인 1인 모드 HOST PASS 2136 checks. 두 역할의 조작, 새 네 방, 두 인계 경로와 안전 검사 통과. 멀티 검사와 독립된 소켓 없는 프로세스에서 실행했다.
- `Logs/gravity-expansion-preview.log`: 최종 저장 씬 렌더 성공. `Logs/gravity-redesign-puzzle-9.png`~`gravity-redesign-puzzle-12.png`에서 배치와 표식을 확인했다. 격벽은 엷은 하늘색 반투명 면, 창구는 초록 테두리로 구분하고 초기 상자/배치 구역 글자 겹침을 줄였다.
- 실행 파일과 씬 갱신 완료. Unity가 자동 변경한 무관한 렌더/프로젝트 설정은 복원했다. 새 네 방의 두 사람 재미와 실제 인터넷 Relay 지연은 아직 사람 플레이테스트로 확인해야 한다. 모든 우회 풀이를 막지는 않았으며 10번의 두 손잡이 사전 준비도 허용한다.
